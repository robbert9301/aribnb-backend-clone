    ...
    def post(self, request):
        ...
        if request.user.is_authenticated:
            ...
            if serializer.is_valid():
                ...
                try:
                    with transaction.atomic(): #db에 즉시 반영되지않고 완벽하게 valid해야 넣음
                        room = serializer.save(
                            owner=request.user,
                            category = category,
                        )
                    # step 1 create room stetp 2 add amenities to room
                    ...
                    if amenities:
                        ...
                        for amenity_pk in amenities:
                            try:
                                ...
                                raise ParseError("Amenity not found")
                        updated_room.amenities.set(amenity_list)
                    serializer.save()
                    return Response(serializer.data)
            except Exception:
...
class RoomBooking(APIView):
    ...
    def get(self, request, pk):
        ...
        bookings = Booking.objects.filter(
            ...
            check_in__gt=now, #시간 지난거는 안보여주기 위해서
        )
    ...
    def post(self, request, pk):
        ...
        if serializer.is_valid():
            ...
            return Response(serializer.data) #Response({"ok": True})
        else:
            return Response(serializer.errors)